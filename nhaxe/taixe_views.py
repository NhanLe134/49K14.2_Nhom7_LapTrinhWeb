...
def quanlytaixe(request):
    ...
    try:
        ...
        # 2. Lấy danh sách tài khoản người dùng để lấy TenDangNhap và SoDienThoai
        ...
        if res_taixe.status_code == 200 and res_users.status_code == 200:
            taixe_raw = res_taixe.json()
            users_raw = res_users.json()
            
            # Tạo map user theo UserID để tra cứu nhanh
            users_map = {u.get('UserID'): u for u in users_raw}

            for tx in taixe_raw:
                tx_id = tx.get('TaixeID', '')
                user_info = users_map.get(tx_id, {})
